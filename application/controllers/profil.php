...
age' => $this->unread_message, 'menus' => $this->menus, 'rolename' => $this->auth->get_rolename(), 'nama_user' => $this->auth->get_user_data()->nama, 'pendidikan' => $pendidikan));
    ...
    function sosmed() {
        ...
        $template_header = 'templates/responsive/header';
        $template_body = 'templates/responsive/profil/sosmed';
        $template_bottom = 'templates/responsive/footer_home';

        $sosmed = $this->profil_model->sosmed($this->id);
        //dump($biodata);die();
        $this->load->view($template_header, array('aplikasi' => $this->aplikasi, 'query_pesan' => $this->query_pesan, 'query_pesan_unread' => $this->query_pesan_unread));
        $this->load->view($template_body, array('aplikasi' => $this->aplikasi, 'unread_message' => $this->unread_message, 'menus' => $this->menus, 'rolename' => $this->auth->get_rolename(), 'nama_user' => $this->auth->get_user_data()->nama, 'sosmed' => $sosmed));
        $this->load->view($template_bottom, array('aplikasi' => $this->aplikasi));
    ...
    function edit_sosmed() {
        ...
        $this->db->where('id', 99);
        if ($this->db->update('t_sosmed', $data)) {
            $this->session->set_flashdata('result', 'Update sosmed success');
            $this->session->set_flashdata('mode_alert', 'success');
            redirect('profil/sosmed');
        } else {
            return false;
        }
    }
